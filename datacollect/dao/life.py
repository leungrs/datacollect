...
def select_by_id(db, id):
    sql = "select * from life_survey where id=?"
    life = db.execute(sql, [id]).fetchone()
    item = sqlite3_row_to_dict(life)
    return item
...
def delete_by_id(db, id):
    ...
    if item:
        sql = "delete from life_survey where id=?"
        db.execute(sql, [id])
        db.commit()
        return True
    return False
...
def insert_update(db, param, id=None):
    try:
        ...
        if id is not None:
            sql = "update life_survey set "
            sql += ",".join("{0}=?".format(f) for f in fields)
            sql += " where id=?"
            values.append(id)
        else:
            sql = "insert into life_survey({0})".format(",".join(fields))
...
def import_life_from_array(array, db, updated_date, updated_by):
    """导入餐饮表，返回导入的记录数"""
    columns = array[0]
    rows = array[1:]
    valid_names = {
