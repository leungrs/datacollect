...
from datacollect.dao import river, select
...
@login_required
def index():
    return render_template('river/index.html')
...
def create():
    ...
    if request.method == 'POST':
        ...
        error = river.insert_update(get_db(), item)
        if error is not None:
            flash(error)
        else:
            return redirect(url_for('river.index'))
    ADMIN_HANDLER.select_by_item(item)
    admins = ADMIN_HANDLER.get_admins()
    return render_template('river/update.html', item=item, admins=admins)
...
def update(id):
    ...
    if request.method == 'POST':
        item = request.form
        error = river.insert_update(get_db(), item, id)
        if error is not None:
            flash(error)
        else:
            return redirect(url_for('river.index'))
    else:
        item = river.select_by_id(get_db(), id)
    ADMIN_HANDLER.select_by_item(item)
    admins = ADMIN_HANDLER.get_admins()
    return render_template('river/update.html', item=item, admins=admins)
...
def export(id):
    item = river.select_by_id(get_db(), id)
    result = render_template("river/rest.xml", item=item)
    response = make_response(result)
    response.headers["Content-Disposition"] = "attachment; filename=river.doc"
    return response
