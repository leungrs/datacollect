...
from datacollect.dao import life, select
...
@login_required
def index():
    return render_template('life/index.html')
...
def create():
    ...
    if request.method == 'POST':
        ...
        error = life.insert_update(get_db(), item)
    ...
    return render_template('life/update.html', item=item, admins=admins)
...
def update(id):
    ...
    if request.method == 'POST':
        item = request.form
        error = life.insert_update(get_db(), item, id)
        if error is not None:
            flash(error)
        else:
            return redirect(url_for('life.index'))
    else:
        item = life.select_by_id(get_db(), id)
    ADMIN_HANDLER.select_by_item(item)
    admins = ADMIN_HANDLER.get_admins()
    return render_template('life/update.html', item=item, admins=admins)
...
def export(id):
    item = life.select_by_id(get_db(), id)
    result = render_template("life/rest.xml", item=item)
    response = make_response(result)
    response.headers["Content-Disposition"] = "attachment; filename=life.doc"
    return response
