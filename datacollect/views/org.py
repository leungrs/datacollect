...
from datacollect.dao import org, select
...
def create():
    ...
    if request.method == 'POST':
        ...
        error = org.insert_update(get_db(), item)
    ...
    return render_template('org/update.html', item=item, admins=admins)
...
def update(id):
    ...
    if request.method == 'POST':
        item = request.form
        error = org.insert_update(get_db(), item, id)
        if error is not None:
            flash(error)
        else:
            return redirect(url_for('org.index'))
    else:
        item = org.select_by_id(get_db(), id)
    ADMIN_HANDLER.select_by_item(item)
    admins = ADMIN_HANDLER.get_admins()
    return render_template('org/update.html', item=item, admins=admins)
...
def export(id):
    item = org.select_by_id(get_db(), id)
    result = render_template("org/rest.xml", item=item)
    response = make_response(result)
    response.headers["Content-Disposition"] = "attachment; filename=org.doc"
    return response
